...
<tr>
	<td class="title1" bgcolor="$color{'titlebg'}"><p>$txt{'125'}</p></td>
</tr>
<tr>
	<td class="form1" bgcolor="$color{'windowbg'}"><p>
$txt{'212'}<br>
<a href="$cgi&action=notify3&thread=$INFO{'thread'}">$txt{'163'}</a> - <a href="$cgi&action=display&num=$INFO{'thread'}">$txt{'164'}</a>
</p></td>
</tr>
</table>
EOT
...
<tr>
	<td class="form1" bgcolor="$color{'windowbg'}"><p>
$txt{'126'}<br>
<a href="$cgi&action=notify2&thread=$INFO{'thread'}">$txt{'163'}</a> - <a href="$cgi&action=display&num=$INFO{'thread'}">$txt{'164'}</a>
</p></td>
</tr>
</table>
EOT
...
EOT
	...
	if (@found_number==0) {
		...
		print "<table>\n";
		print "<tr><td colspan=2><p>$txt{'415'}:</p><br>&nbsp;</td></tr>";
		$counter=0;
		foreach $entry (@found_number) {
			print "<tr><td><p>";
			print "<input type=checkbox name=\"thread-$found_number[$counter]\" value=\"1\"></p></td>";
			print "<td><p><i>$found_subject[$counter]</i> by <a href=\"$boardurl/YaBB.pl?board=&action=viewprofile&username=$found_username[$counter]\">$found_name[$counter]</a></p></td></tr>\n";
			$counter++;
		}
		print "<tr><td colspan=2><br><p>$txt{'416'}</p><br>&nbsp;</td></tr>\n";
