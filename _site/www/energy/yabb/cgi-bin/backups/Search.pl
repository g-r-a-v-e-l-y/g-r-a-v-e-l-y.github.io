...
sub setup { # start setup
    ...
    @catlist = grep(/\.cat$/,@dirdata);
    $totalboards = @sorteddirdata;
    
	#aaron
    foreach $category (@catlist) { # start to open files
		...
		if($group eq $catgroup || $catgroup eq ""){
			$SIZE=@catdata;
			for ($i=2;$i<$SIZE;$i++) {
				$board = $catdata[$i];
				chomp $board;

				open(FILE, "$boardsdir/$board.dat") || next;
				&lock(FILE);
				@boarddat = <FILE>;
				&unlock(FILE);
				close(FILE);
				$selecthtml.="<option value=\"$board.txt\">$boarddat[0]\n";
			}
		}
