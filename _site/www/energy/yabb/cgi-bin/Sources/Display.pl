...
<a href="$cgi">$boardname</a>: <a href="$cgi&action=display&num=$viewnum">$msub[0]</a></b></font></td>
	<td align=right><p><a href="Printpage.pl?board=$currentboard&num=$viewnum" target="_blank">print page</a><br><a href="$cgi&action=post&num=$viewnum&title=$txt{'116'}&start=$start"><p>reply</P></a>$notification</td>
</tr>
</table>
	...
	for ($a = $start; $a < @messages; $a++) {
		...
		if($musername[$a] ne "Guest" && $removed==0) {
			if($username ne "Guest") {
				$sendm = " <a href=\"$cgi\&action=imsend\&to=$musername[$a]\">e-mail</a>";
			}
...
<tr>
	<td bgcolor="$second"><table border=0 cellspacing=0 cellpadding=0 width=500><tr><td><font size=1>
$url <a href="mailto:$memail[$a]">e-mail</a>$viewd$sendm
</font></td><td align=right>
<font size=1><a href="$cgi&action=post&num=$viewnum&quote=$a&title=$txt{'116'}&start=$start">quote</a> <a href="$cgi&action=modify&message=$a&thread=$viewnum">modify</a> <a href="$cgi&action=modify2&thread=$viewnum&id=$a&d=1">delete</a></font></td></tr></table></td>
</tr>
EOT
;
