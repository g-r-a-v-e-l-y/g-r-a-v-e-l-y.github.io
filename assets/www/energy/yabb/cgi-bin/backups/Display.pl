...
EOT
	for ($a = $start; $a < @messages; $a++) {
		...
		if($musername[$a] ne "Guest" && $removed==0) {
			...
			$viewd = " <a href=\"$cgi\&action=viewprofile\&username=$musername[$a]\"><img src=\"$imagesdir/profile.gif\" width=44 height=16 alt=\"Profile\" border=0></a>";
...
<div align=right><form action="YaBB.pl" method="GET">
<font size=1>$txt{'160'}: <select name="board">$selecthtml</select> <input type=submit value="$txt{'161'}"></form></div>
EOT
	&footer;
	exit;
}
