...
sub LastPost {
	...
	foreach $curcat (@categories) {
		...
		foreach $curboard (@catboards ) {
			...
			$post = qq~$txt{'234'} "<a href="$cgi$curboard&action=display&num=$mnum">$msub</a>" $txt{'235'} ($mydatetime)<br></p></td>\n~;
