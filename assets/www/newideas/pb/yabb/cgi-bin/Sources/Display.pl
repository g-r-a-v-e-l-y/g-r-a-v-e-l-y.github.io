...
<table width=100% cellpadding=0 cellspacing=0>
  <tr>
    <td valign=bottom colspan="2"><span class="forumtree">
    ...
    <td bgcolor="$color{'catbg'}" align="left" valign="middle">$txt{'139'}: $pageindex
    </td>
    <td bgcolor="$color{'catbg'}" align="right" valign="middle"><a class="replynote" href="$cgi&action=post&num=$viewnum&title=$txt{'116'}&start=$start">reply</a>  $notify<a href="Printpage.pl?board=$currentboard&num=$viewnum" target="_blank">print</a></td>
  </tr>
</table>
<table border=0 width=100% cellspacing=1 cellpadding=3  bgcolor="$color{'bordercolor'}">
  <tr>
    <td class="title1" bgcolor="$color{'titlebg'}"><table border=0 cellspacing=0 cellpadding=0><tr><td><p><img src="$imagesdir/$threadclass.gif"></td><td><p>&nbsp;$txt{'29'}</p></td></tr></table></td>
    <td class="title1" bgcolor="$color{'titlebg'}"><p>$txt{'118'}: $msubthread</p></td>
  </tr>
EOT
	...
	foreach (@messages) {
		...
		if( $mlm ) {
			$mlm = &timeformat($mlm);
			$lastmodified = qq~<BR><hr size="1" noshade width="90%" align="center"><i>$txt{'211'}: $mlmb - $mlm</i>~;
		}
		else {
			$mlm = '-';
			$lastmodified = '';
		...
		if( $musername ne 'Guest' && ( $userprofile{$musername} || -e("$memberdir/$musername.dat") ) ) {
			if( $username ne 'Guest' ) {
				$sendm = qq~ <a href="$cgi&action=imsend&to=$musername">private message</a>~;
			}
			if( exists $userprofile{$musername} ) {
				...
				$userprofile[4] = $userprofile[4] && $userprofile[4] ne q~http://~ ? qq~<a href="$userprofile[4]" target="_blank">website</a>~ : '';
				...
				if( $showgenderimage ) {
					...
					$userprofile[11] = $userprofile[11] ? qq~<p>$txt{'231'}: <img src="$imagesdir/$userprofile[11].gif" border="0" alt="$userprofile[11]"><br>~ : '';
				}
			...
			$usernamelink = qq~<a href="$scripturl?board=$currentboard&action=viewprofile&username=$musername"><p>$mname</p></a>~;
			$postinfo = qq~$txt{'26'}: $userprofile[6]<br>~;
			$memail = $userprofile[2];
		}
		else {
			$usernamelink = qq~<p>$mname</p>~;
			$postinfo = '';
			$star = '';
			$memberinfo = $txt{'28'};
			$icqad = '';
		...
		print << "EOT";
<tr>
<td bgcolor="$windowbg" width=140 valign=top rowspan=2><span class="blacky">
	$usernamelink<br>
	$memberinfo<br>
	$star<br><br>
	$postinfo<br>
...
<td bgcolor="$windowbg" valign=top height="100%">
	...
	<td valign="middle"><span class="blacky"> &nbsp;<img src="$imagesdir/$micon.gif">&nbsp;<a name="$counter"></a>
	$msub &nbsp;&nbsp;($txt{'30'}: $messdate)</span></td>
	<td align="right" valign="middle"><span class="blacky"$txt{'512'}: $mip</span></td></tr></table>
...
<tr height="16">
	...
	&nbsp; $userprofile[4] &nbsp;<a href="mailto:$memail">email</a> &nbsp;$sendm
	</td>
	<td align=right nowrap valign="middle"><p>$txt{'146'} #$counter</p> <a href="$cgi&action=post&num=$viewnum&quote=$counter&title=$txt{'116'}&start=$start">quote</a> <a href="$cgi&action=modify&message=$counter&thread=$viewnum">modify</a>
EOT
		if(exists $moderators{$username} || $settings[7] eq 'Administrator') {
print << "EOT";
<a href="$cgi&action=modify2&thread=$viewnum&id=$counter&d=1">delete</a></p></td>
EOT
}
print << "EOT";
</td></tr></table></td>
...
<table border=0 width=100% cellpadding=0 cellspacing=0>
  <tr height="30">
    <td bgcolor="$color{'catbg'}" align="left" colspan="2"><p>$txt{'139'}:
$pageindex
    </p></td>
...
EOT
    ...
    <p>$txt{'160'}: <select name="board">$selecthtml</select> <input type=submit value="$txt{'161'}"></form></td>
  </tr>
</table>
EOT
	&footer;
