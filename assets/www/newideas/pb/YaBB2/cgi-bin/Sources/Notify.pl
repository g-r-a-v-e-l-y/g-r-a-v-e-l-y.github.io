...
<tr>
	<td class="title1" bgcolor="$color{'titlebg'}"><font size=2 class="text1" color="$color{'titletext'}">$txt{'125'}</font></td>
</tr>
<tr>
...
<table border=0 width=100% cellspacing=1 cellpadding=6 bgcolor="#000000">
 <tr>
  <td bgcolor="$color{'titlebg'}">
   <font size=2 color="$color{'titletext'}">$txt{'418'}</font>
  </td>
 </tr>
 <tr>
...
EOT
	...
	if (@found_number==0) {
		...
		foreach $entry (@found_number) {        
			...
			print "<td><font size=2><i>$found_subject[$counter]</i> by <a href=\"$boardurl/YaBB.pl?board=&action=viewprofile&username=$found_username[$counter]\">$found_name[$counter]</a></font></td></tr>\n";
			$counter++;
		}
