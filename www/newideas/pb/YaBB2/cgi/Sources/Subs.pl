...
sub header {
	...
	if($MenuType == 1) {
		$yymenu = qq~<font size=2><a href="YaBB.pl">$txt{'103'}</a> | <a href="$helpfile" target=_blank>$txt{'119'}</a> | <a href="$cgi\&action=search">$txt{'182'}</a> | ~;
		if($settings[7] eq 'Administrator') { $yymenu .= qq~<a href="$cgi\&action=admin">$txt{'2'}</a> | ~; }
		if($username eq 'Guest') { $yymenu .= qq~<a href="$cgi\&action=login">$txt{'34'}</a> | <a href="$cgi&action=register">$txt{'97'}</a></font>~;
		} else { $yymenu .= qq~<a href="$cgi\&action=profile\&username=$username">$txt{'79'}</a> | <a href="$cgi\&action=shownotify">$txt{'419'}</a> | <a href="$cgi\&action=logout">$txt{'108'}</a></font>~; }
	}
	...
	if($enable_news) {
		...
		close(FILE);
		srand;
		$yynews = "<b>$txt{'102'}\:</b> $newsmessages[int rand(@newsmessages)]";
	}
	...
	$yyuname = $username eq 'Guest' ? qq~$txt{'248'} <b>$txt{'28'}</b>. $txt{'249'}~ : qq~$txt{'247'} <b>$realname</b>, ~ ;
...
<tr>
	<td class="title1" bgcolor="$color{'titlebg'}"><font size=2 class="text1" color="$color{'titletext'}"><b>$txt{'106'}</b></font></td>
</tr>
<tr>
	<td class="form1" bgcolor="$color{'windowbg'}"><font size=2>$e</font></td>
</tr>
</table>
...
if($sender ne "News") {
	...
	$message =~ s~\[quote\s+author=(.*?)link=(.*?)\s+date=(.*?)\s*\](.*?)\[/quote\]~<blockquote>on $3, $1 wrote:<hr><b>$4</b><hr><a href="$scripturl?action=display&$2">\($txt{'516'}\)</a></blockquote>~isg;
	...
	$message =~ s~\[code\](.+?)\[/code\]~<blockquote><font size=2 face="Courier New">code:</font><hr><font size="2" face="Courier New"><pre>$1</pre></font><hr></blockquote>~isg;
}
	while( $message =~ s~<a([^>]*?)\n([^>]*)>~<a$1$2>~ ) {}
	while( $message =~ s~<a([^>]*)>([^<]*?)\n([^<]*)</a>~<a$1>$2$3</a>~ ) {}
